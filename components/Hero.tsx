    ...
    <section className="min-h-screen flex flex-col justify-center relative px-6 pt-20 overflow-hidden">
      ...
      >
        <div className="overflow-hidden">
          <motion.h2 variants={item} className="text-xl md:text-2xl font-light text-blue-400 mb-4 tracking-widest uppercase drop-shadow-md">
            {t.role[language]}
          </motion.h2>
        </div>
        
        <div className="overflow-hidden">
          <motion.h1 variants={item} className="text-7xl md:text-[10rem] font-display font-bold leading-[0.9] tracking-tighter mb-6 drop-shadow-2xl text-white">
            {titleFirst}<br />
            <span className="text-gray-300">{titleSecond}</span>
          </motion.h1>
        </div>

        <div className="flex flex-col md:flex-row justify-between items-end mt-12 md:mt-24">
          <motion.div variants={item} className="max-w-md p-6 bg-black/40 backdrop-blur-md rounded-xl border border-white/5 shadow-2xl">
            <p className="text-lg md:text-xl text-gray-100 leading-relaxed shadow-black drop-shadow-md font-medium">
              {t.subtitle[language]}
            </p>
          </motion.div>

          <motion.div variants={item} className="mt-12 md:mt-0">
             <MagneticButton className="group flex items-center gap-4 interactive">
                ...
                <span className="uppercase tracking-widest text-sm drop-shadow-md font-bold">{t.cta[language]}</span>
             </MagneticButton>
          </motion.div>
        </div>
      </motion.div>
